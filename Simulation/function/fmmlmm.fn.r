...
FMMLMM.EM = function(Data, X, Z, g=g, init.para, cor.type = c("UNC", "DEC", "CAR1", "ARp"), true.cls=NULL, tol = 1e-6, max.iter=500, per=100)
{
  begin = proc.time()[1]
  p = ncol(X)
  q = ncol(Z)
...
# Parameter estimation
  ...
  est.out = MI.fmmlmm(para.est, cor.type=cor.type)
# Model selection
...
# information matrix
MI.fmmlmm = function(para.est, cor.type=c("UNC", "DEC", "CAR1", "ARp"))
{
   w = para.est$w
   Beta = para.est$Beta
   DD = para.est$DD
