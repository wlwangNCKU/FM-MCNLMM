...
EFMMLMM.EM = function(Data, X, Z, V, g=g, init.para, cor.type = c("UNC", "DEC", "CAR1", "ARp"), true.cls=NULL, tol = 1e-6, max.iter=500, per=100)
{
  begin = proc.time()[1]
  p = ncol(X)
  q = ncol(Z)
  ...
  y = NULL
  if(r == 1){ for(j in 1: n) y = c(y, Data$Var1[Data$Subject == j])
  } else{
...
# Parameter estimation
  ...
  est.out = MI.efmmlmm(para.est, cor.type=cor.type)
...
# information matrix
MI.efmmlmm = function(para.est, cor.type=c("UNC", "DEC", "CAR1", "ARp"))
{
   Beta = para.est$Beta
   DD = para.est$DD
