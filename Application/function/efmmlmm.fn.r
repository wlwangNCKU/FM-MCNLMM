####### EM Algorithm for Extended Finite Mixture of MLMM #######
EFMMLMM.EM = function(Data, y, X, Z, V, g=g, init.para, cor.type = c("UNC", "DEC", "CAR1", "ARp"), true.cls=NULL, tol = 1e-6, max.iter=500, per=10)
{
  begin = proc.time()[1]
...
#observed log-likelihood
  ...
  for(i in 1: g){for(j in 1: n){
      ...
      if(length(idx)==1) wden[j,i] = w[j, i] * dnorm(y[idx], X[idx, ]%*%Beta[,i], sd=sqrt(TLam[[i]][idx, idx]))
      else wden[j,i] = w[j, i] * dmvnorm(y[idx], X[idx, ]%*%Beta[,i], TLam[[i]][idx, idx])
  }}
  ...
  cat("Extended finite mixture of multivariate linear mixed model with ", g, "components and ", cor.type[1], " errors: ", "\n")
...
# Sigma
    ...
    if(cor.type == "CAR1"){
     ga[i] = 1
     Phi[i] = optim(par = Phi[i], fn = phi.Eloglik, method = "L-BFGS-B", lower = 1e-6, upper = 1-1e-6,
                    gup=i, y=y, w=w, Beta=Beta, DD=DD, Sigma=Sigma, Phi=Phi, ga=ga, X=X, Z=Z, cor.type=cor.type, sj=sj, cumsum.nj=cumsum.nj)$par
    }
    if(cor.type == "DEC"){
     DEC.est = optim(par = c(Phi[i], ga[i]), fn = phi.Eloglik, method = "L-BFGS-B", lower = c(1e-6, 1e-6), upper = c(1-1e-6, Inf),
                    gup=i, y=y, w=w, Beta=Beta, DD=DD, Sigma=Sigma, Phi=Phi, ga=ga, X=X, Z=Z, cor.type=cor.type, sj=sj, cumsum.nj=cumsum.nj)$par
     Phi[i] = DEC.est[1]; ga[i] = DEC.est[2]
    }
    if(cor.type == "ARp"){
    P = length(Phi[i])
     Phi[i] = optim(par = Phi[i], fn = phi.Eloglik, method = "L-BFGS-B", lower = rep(-1 + 1e-6, P), upper = rep(1 - 1e-6, P),
                    gup=i, y=y, w=w, Beta=Beta, DD=DD, Sigma=Sigma, Phi=Phi, ga=ga, X=X, Z=Z, cor.type=cor.type, sj=sj, cumsum.nj=cumsum.nj)$par
    }}
...
#observed log-likelihood
   ...
   for(i in 1: g){
    ...
    para = c(para, as.vector(Phi), ga, psi)
    if(iter%%per == 0) cat("iter = ", iter, ",\t obs.loglik = ", loglik.new, ",\t diff = ", diff, ",\t psi = ", psi, ",\t phi = ", Phi, ",\t ga = ", ga, sep = " ", "\n")
    if(diff < tol || iter >= max.iter) break
    loglik.old = loglik.new
  }
  end = proc.time()[1]
# Parameter estimation
  cat(rep("=", 20), "Extended Finite mixture of linear mixed models with ", cor.type[1], " errors", rep("=", 20), sep = "", "\n")
  cat("It took", end - begin, "seconds.\n")
  cat("iter = ", iter, ",\t obs.loglik = ", loglik.new, sep = "", "\n")
  para.est = list(Beta = Beta, Sigma = Sigma, DD = DD, Phi = Phi, ga = ga, psi = psi, para = para)
  est.out = MI.efmmlmm(para.est, y, cor.type=cor.type)

# Model selection
  v = ncol(V)*(g-1)
  if(cor.type == "UNC"){ m = g * (p + q*(q+1)/2 + r*(r+1)/2) + v
  } else if(cor.type == "DEC"){ m = g * (p + q*(q+1)/2 + r*(r+1)/2) + length(as.vector(Phi)) + length(ga) + v
  } else{ m = g * (p + q*(q+1)/2 + r*(r+1)/2) + length(as.vector(Phi)) + v}
...
phi.Eloglik = function(phiga, gup, y, w, Beta, DD, Sigma, Phi, ga, X=X, Z=Z, cor.type, sj, cumsum.nj)
{
...
# information matrix
MI.efmmlmm = function(para.est, y, cor.type=c("UNC", "DEC", "CAR1", "ARp"))
{
   Beta = para.est$Beta
   DD = para.est$DD
   ...
   Si[[i]][1:p, j] = U[j, i] * t(matrix(X[idx, ], ncol=p)) %*% TLam.inv[[i]][idx, idx] %*% y.cent[idx, i]
   for(s in 1: m1) Si[[i]][(p+s), j] = (U[j, i] * sum(diag(y.cent[idx, i] %*% t(y.cent[idx, i]) %*% Linv.dotL[[s]][idx, idx] %*% TLam.inv[[i]][idx, idx])) - U[j, i] * sum(diag(Linv.dotL[[s]][idx, idx])) )/2
   }}
